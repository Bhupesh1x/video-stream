...
export const playlistsRouter = createTRPCRouter({
  ...
  addVideo: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existingPlaylistVideo] = await db
        .select({ id: playlistVideos.playlistId })
        .from(playlistVideos)
        .where(
          and(
  ...
  getCustomPlayistVideos: protectedProcedure
    .input(
      z.object({
        cursor: z
          .object({
    ...
    .query(async ({ input, ctx }) => {
      ...
      const videodFromPlaylist = db.$with("playlist_videos").as(
        db
          .select({
      ...
      const data = await db
        ...
        .innerJoin(
          videodFromPlaylist,
          eq(videodFromPlaylist.videoId, videos.id)
        )
        .where(
          and(
            eq(videos.visibility, "public"),
